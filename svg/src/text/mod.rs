use font::{
    Glyph,
    GlyphId,
    opentype::{ OpenTypeFont, Tag, gsub::{ GSub, Substitution, LanguageSystem }, gdef::MarkClass },
};
// #[cfg(feature="svg")]
use crate::types::SvgGlyph;

pub use font::FontError;
use pathfinder_geometry::{ vector::{ Vector2F, vec2f }, transform2d::Transform2F, rect::RectF };
use std::sync::Arc;
...
fn apply_subs<'a, 'b>(
    glyphs: &'a mut Vec<(usize, GlyphId)>,
    pos: usize,
    subs: impl Iterator<Item = &'b Substitution>
) -> (usize, isize) {
    for sub in subs {
        ...
        match *sub {
            Substitution::Single(ref map) => {
                if let Some(&replacement) = map.get(&(first as u16)) {
                    log::debug!("replace gid {:?} with {:?}", glyphs[pos], GlyphId(replacement as u32));
            ...
            Substitution::Ligatures(ref map) => {
                if let Some(subs) = map.get(&(first as u16)) {
                    for &(ref sub, glyph) in subs {
                        if
                            let Some(len) = sub.matches(
                                glyphs[pos + 1..].iter().map(|&(_, gid)| gid)
                            )
                        {
                            log::debug!(
                                "ligature {}..{} with {:?}",
                                pos,
                                pos + len + 1,
                                GlyphId(glyph as u32)
                            );
                            glyphs.splice(
                                pos..pos + len + 1,
                                std::iter::once((first_idx, GlyphId(glyph as u32)))
                            );
    ...
    where F: Fn(&MetaGlyph) -> G, G: Fn(Tag) -> bool
{
    ...
    while let Some(m) = meta.get(meta_pos as usize) {
        log::debug!(
            "pos {}, meta_pos: {}, gids[pos] = {:?}, meta[meta_pos] = {:?}",
            pos,
            meta_pos,
            gids[pos],
            m
...
fn process_chunk(
    ...
    if let Some(fm) = font.vmetrics() {
        ...
        state.vmetrics = match state.vmetrics {
            ...
            Some(m1) =>
                Some(VMetrics {
                    ascent: m1.ascent.max(vm.ascent),
                    descent: m1.descent.min(vm.descent),
                }),
        };
    }

    for g in meta {
        log::debug!("at byte {} [\u{2068}{}\u{2069} 0x{:x}]", g.idx, g.codepoint, g.codepoint as u32);
    }
    // (codepoint idx, glyph id)
    let mut gids: Vec<(usize, GlyphId)> = meta
        .iter()
        .filter(|&m| {
            match m.category {
    ...
    if let Some(gsub) = gsub {
        if let Some(lang) = language.and_then(|s| gsub.language(s)).or(gsub.default_language()) {
            ...
            sub_pass(
                ...
                meta,
                &mut gids,
                |m| |tag| [Tag(*b"rlig"), Tag(*b"liga")].contains(&tag)
            );
        }
    }
    ...
    for (index, gid) in gids {
        if let Some(glyph) = font.glyph(gid) {
            let mark = match
                (
                    gdef
                        .and_then(|gdef| gdef.mark_class(gid.0 as u16))
                        .unwrap_or(MarkClass::Unassigned),
                    last_gid,
                )
            {
                ...
                None => {
                    let kerning =
                        ...
                        vec2f(
                            last_gid
                                .replace(gid)
                                .map(|left| font.kerning(left, gid))
                                .unwrap_or_default(),
                            0.0
                        );
...
impl FontCollection {
    pub fn layout_run(&self, string: &str, rtl: bool, lang: Option<Language>) -> Layout {
        ...
        for (word_off, word) in WordBoundIndices::new(string) {
            ...
            let mut meta: Vec<MetaGlyph> = word
                .char_indices()
                .map(|(idx, c)| MetaGlyph::new(c, word_off + idx))
                .collect();
            ...
            if let Some((font_idx, font)) = font_for_text(fonts, word, &meta) {
                ...
                for (idx, grapheme) in GraphemeIndices::new(word) {
                    ...
                    if
                        let Some((font_idx, font)) = font_for_text(
                            fonts,
                            grapheme,
                            &meta[meta_idx..meta_idx + meta_len]
                        )
                    {
        ...
        let (font_bounding_box_ascent, font_bounding_box_descent) = fonts
            .iter()
            .filter_map(|f| {
                let s = f.font_matrix().m22();
                f.vmetrics().map(|m| (s * m.ascent, s * m.descent))
            })
            .fold1(|(a1, d1), (a2, d2)| (a1.max(a2), d1.min(d2)))
            .unwrap_or((0.0, 0.0));
